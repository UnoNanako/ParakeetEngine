...
void ParticleCommon::Initialize(std::shared_ptr<DirectXCommon> dxCommon) {
	...
	HRESULT pHr;
	pHr = D3D12SerializeRootSignature(&descriptionRootSignature, D3D_ROOT_SIGNATURE_VERSION_1, &signatureBlob, &errorBlob);
	if (FAILED(pHr)) {
	...
	pHr = dxCommon->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
	...
	particleInputLayoutDesc.pInputElementDescs = inputElementDescs;
	particleInputLayoutDesc.NumElements = _countof(inputElementDescs);
	...
	graphicsPipelineStateDesc.InputLayout = particleInputLayoutDesc;// InputLayout
	...
	// DepthStencilの設定
	graphicsPipelineStateDesc.DepthStencilState = particleDepthStencilDesc;
	graphicsPipelineStateDesc.DSVFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;

	// 実際に生成
	pHr = dxCommon->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&mGraphicsPipelineState));
	assert(SUCCEEDED(pHr));
...
void ParticleCommon::PreDraw(std::shared_ptr<DirectXCommon> dxCommon, std::shared_ptr<Camera> camera) {
	...
	*mViewProjectionMatrixData = viewProjectionMatrix;
	...
	dxCommon->GetCommandList()->SetPipelineState(mGraphicsPipelineState.Get());
	dxCommon->GetCommandList()->SetGraphicsRootConstantBufferView(2, mViewProjectionMatrixResource->GetGPUVirtualAddress());
}

void ParticleCommon::PostDraw(std::shared_ptr<DirectXCommon> dxCommon) {
