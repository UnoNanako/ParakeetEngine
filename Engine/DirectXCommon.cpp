...
void DirectXCommon::PreDraw() {
	...
	//--------------------レンダーターゲットと深度をクリア--------------------
	// 指定した色で画面全体をクリアする
	float clearColor[] = { 0.2f, 0.2f, 1.0f, 1.0f }; // 青っぽい色。RGBAの順
...
void DirectXCommon::PostDraw() {
	...
	//--------------------GPUの実行を待つ(fenceは実行を待つものではなく、GPUの処理が終わったかを調べるもの)--------------------
	//Fenceの値を更新
	mFenceValue++;
...
void DirectXCommon::CreateSwapChain(std::shared_ptr<WinApp> winApp) {
	//スワップチェーンを生成する
	...
	HRESULT hr = mDxgiFactory->CreateSwapChainForHwnd(mCommandQueue.Get(), winApp->GetHwnd(), &swapChainDesc, nullptr, nullptr, reinterpret_cast<IDXGISwapChain1**>(mSwapChain.GetAddressOf()));
	assert(SUCCEEDED(hr));
}
...
void DirectXCommon::CreateEachDescriptorHeap() {
	...
	mSrvDescriptorHeap = CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 1024, true);
...
void DirectXCommon::InitializeViewport() {
	...
	mViewport.Width = WinApp::kClientWidth;
	mViewport.Height = WinApp::kClientHeight;
...
void DirectXCommon::InitializeScissor() {
	...
	mScissorRect.left = 0;
	mScissorRect.right = WinApp::kClientWidth;
	mScissorRect.top = 0;
	mScissorRect.bottom = WinApp::kClientHeight;
}
...
Microsoft::WRL::ComPtr<IDxcBlob> DirectXCommon::CompileShader(const std::wstring& filePath, const wchar_t* profile){
	...
	HRESULT hr = mDxcUtils->LoadFile(filePath.c_str(), nullptr, &shaderSource);
	//読めなかったら止める
	assert(SUCCEEDED(hr));
	//読み込んだファイルの内容を設定する
...
D3D12_CPU_DESCRIPTOR_HANDLE DirectXCommon::GetCPUDescriptorHandle(){
	...
	handleCPU.ptr += (mDescriptorSizeSRV * mSrvDescHeapIndex);
	return handleCPU;
}
...
D3D12_GPU_DESCRIPTOR_HANDLE DirectXCommon::GetGPUDescriptorHandle(){
	...
	handleGPU.ptr += (mDescriptorSizeSRV * mSrvDescHeapIndex);
	return handleGPU;
}
